    ...
    systemd.services."penumbra.cometbft" = {
      ...
      serviceConfig = with strings; let
        ...
        configToml = (pkgs.formats.toml {}).generate "penumbra-cometbft-config" {
          ...
          #   laddr = "tcp://${cfg.rpc.ip}:${cfg.rpc.port}";
          ...
          # };
          # p2p = {
          #   laddr = "tcp://${cfg.p2p.ip}:${cfg.p2p.port}";
          ...
          # };
          # instrumentation = {
          #   prometheus = cfg.prometheus.listenPort ? false;
          #   prometheus_listen_addr = ":${cfg.prometheus.listenPort}";
